...
import { cambiarEstadoEmpleado, crearEmpleado, listarEmpleados, obtenerCatalogosPersonal } from '../../servicios/servicioSegmentoDos.js';
...
export default function GestionEmpleados() {
  ...
  async function guardar(evento) {
    ...
    try {
      ...
      setMensaje({ tipo: 'exito', texto: 'Empleado vinculado.' });
      cargar();
    } catch (error) { setMensaje({ tipo: 'error', texto: error.message }); }
  }
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_2fr]">
        <Tarjeta>
          <form className="space-y-4" onSubmit={guardar}>
            <h2 className="font-semibold text-primary">Vincular empleado</h2>
            <CampoTexto etiqueta="ID de usuario" type="number" min="1" value={formulario.idUsuario} onChange={(e) => setFormulario({ ...formulario, idUsuario: e.target.value })} required />
            ...
            <Boton type="submit">Vincular empleado</Boton>
          </form>
        </Tarjeta>
        <TablaDatos filas={empleados} clave="IdEmpleado" columnas={[
          ...
          { clave: 'acciones', etiqueta: 'Acciones', render: (fila) => <Boton variante="texto" onClick={() => alternar(fila)}>{fila.Activo ? 'Desactivar' : 'Activar'}</Boton> }
        ]} />
      </div>
    </div>
  );
