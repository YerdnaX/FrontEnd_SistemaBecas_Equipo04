  ...
  return (
    <div>
      ...
      <Tarjeta className="mx-auto w-full max-w-5xl">
        <h1 className="text-headline-sm font-semibold text-on-surface">Situación socioeconómica</h1>
        <form className="mt-6 flex flex-col gap-6" onSubmit={manejarEnvio}>
          ...
          <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">
            <CampoSelect
              ...
              etiquetaVacia="Seleccione un tipo"
              opciones={TIPOS_VIVIENDA.map((tipo) => ({ valor: tipo, etiqueta: tipo }))}
              value={formulario.tipoVivienda}
              onChange={(e) => actualizarCampo('tipoVivienda', e.target.value)}
              error={erroresCampos.tipoVivienda}
              required
            />
            ...
            <CampoSelect
              ...
              etiquetaVacia="Seleccione una situación"
              opciones={SITUACIONES_LABORALES.map((situacion) => ({ valor: situacion, etiqueta: situacion }))}
              value={formulario.situacionLaboral}
              onChange={(e) => actualizarCampo('situacionLaboral', e.target.value)}
              error={erroresCampos.situacionLaboral}
              required
            />
