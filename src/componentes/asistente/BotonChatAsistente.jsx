...
export default function BotonChatAsistente() {
  ...
  return (
    <div className="fixed bottom-5 right-5 z-40 flex flex-col items-end gap-3">
      {abierto && (
        <div className="flex h-[28rem] w-[22rem] max-w-[calc(100vw-2.5rem)] flex-col overflow-hidden rounded-lg border border-outline-variant bg-white text-on-surface shadow-elevation-l3">
          <div className="flex items-center justify-between bg-primary-container px-4 py-3 text-on-primary">
            <span className="text-body-md font-semibold">Asistente virtual</span>
            <button type="button" onClick={() => setAbierto(false)} aria-label="Cerrar chat" className="rounded-full px-2 py-0.5 hover:bg-white/10">✕</button>
          </div>
          ...
          <form onSubmit={enviar} className="flex gap-2 border-t border-outline-variant p-2">
            <input
              ...
              maxLength={1000}
              className="flex-1 rounded-md border border-outline-variant px-3 py-2 text-body-sm outline-none focus:border-primary-container disabled:opacity-50"
            />
            <button
              type="submit"
              disabled={enviando || iniciando || !texto.trim()}
              className="rounded-md bg-primary-container px-3 py-2 text-label-md font-semibold text-on-primary transition hover:bg-primary disabled:cursor-not-allowed disabled:opacity-50"
      ...
      <button
        ...
        className="flex h-14 w-14 items-center justify-center rounded-full bg-primary-container text-2xl text-on-primary shadow-elevation-l3 transition hover:bg-primary"
      >
